...
const RecordTests = () => {
  ...
  const testId = localStorage.getItem("record_test_id");
  ...
  const assignedArmIds = test?.test_arms?.map(ta => ta.arm_id) || [];

  const { 
    data: studentsData, 
    isLoading: studentsLoading,
  } = useQuery({
    queryKey: ['students', test?.class_id, assignedArmIds, studentPage, debouncedSearch],
    queryFn: async () => {
      let query = supabase
        .from("students")
    ...
    enabled: !!test?.class_id,
    staleTime: 2 * 60 * 1000,
    keepPreviousData: true,
  });

  // ✅ FIXED: Fetch ONLY submissions for current page students
  const { data: submissionsData } = useQuery({
    queryKey: ['test-submissions', testId, currentStudentIds],
    queryFn: async () => {
    ...
    enabled: !!testId && currentStudentIds.length > 0,
    staleTime: 30 * 1000,
  });

  // ========== MUTATIONS ==========
  
  // ✅ FIXED: Single RPC call instead of select + insert/update
  const saveMutation = useMutation({
    mutationFn: async ({ studentId, score }) => {
      const numericScore = parseInt(score);
      const totalMarks = test?.total_marks || 100;

      const { data, error } = await supabase.rpc('record_test_score', {
        p_test_id: parseInt(testId),
    ...
    onSuccess: (data, variables) => {
      queryClient.setQueryData(
        ['test-submissions', testId, currentStudentIds],
        (old) => {
          const newSub = {
  ...
  // ✅ FIXED: Parallel RPC calls for batch save
  const batchSaveMutation = useMutation({
    mutationFn: async () => {
      ...
      const promises = entries.map(([studentId, score]) => 
        supabase.rpc('record_test_score', {
          p_test_id: parseInt(testId),
          p_student_id: studentId,
  ...
  const isLoading = testLoading || studentsLoading;
    ...
    return (
      <div className="w-full max-w-5xl mx-auto px-4 py-6">
        <div className="bg-red-100 text-red-800 border border-red-300 rounded-md p-4">
          <h2 className="font-bold text-lg mb-2">⚠️ No Test Selected</h2>
          <p>Please select a test from the history page.</p>
        </div>
      </div>
    );
  }
  ...
  if (testError) {
    return (
      <div className="w-full max-w-5xl mx-auto px-4 py-6">
        <div className="bg-red-100 text-red-800 border border-red-300 rounded-md p-4">
          <h2 className="font-bold text-lg mb-2">⚠️ Error Loading Test</h2>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              <h2 className="text-lg font-semibold text-gray-700">
                Test: {test?.test_title || test?.title}
              </h2>
              <p className="text-sm text-gray-500 mt-1">
                Total Marks: <span className="font-medium text-gray-700">{totalMarks}</span>
                {test?.duration_minutes && (
                  <span className="ml-4">Duration: {test.duration_minutes} mins</span>
                )}
              </p>
              <p className="text-sm text-gray-500 mt-1">
                {assignedArmIds.length > 0 ? (
                  <>
                    Arms: {test.test_arms.map(ta => ta.arm?.arm_name).join(", ")}
