...
const ExamCompleted = () => {
  ...
  // Validate location state on mount
  useEffect(() => {
    if (!location.state) {
      navigate('/dashboard/view-exams', { replace: true });
    }
  ...
  const getSecurityBadge = () => {
    ...
    const configs = {
      'exam_hall': { color: 'bg-red-100 text-red-800', label: 'Exam Hall Mode' },
      'strict': { color: 'bg-orange-100 text-orange-800', label: 'Strict Mode' },
      'standard': { color: 'bg-blue-100 text-blue-800', label: 'Standard Mode' }
    };
    const config = configs[securityLevel] || configs['standard'];
    return (
