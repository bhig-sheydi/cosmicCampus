...
//       if (searchTerm) query = query.ilike("test_title", `%${searchTerm}%`);
...
//         query = query.gte("test_date", start.toISOString()).lt("test_date", end.toISOString());
//       }
...
//   useEffect(() => setPage(1), [searchTerm, selectedClass, selectedDate, selectedStatus]);

//   /* ---------------- DEBOUNCED FETCH ---------------- */
//   useEffect(() => {
//     if (!userData?.user_id) return;
//     const debounced = debounce(() => fetchTests(page), 500);
//     debounced();
//     return () => debounced.cancel();
//   }, [page, userData, searchTerm, selectedClass, selectedDate, selectedStatus, classMap, fetchTests]);

//   /* ---------------- INITIAL CLASS LOAD ---------------- */
//   useEffect(() => fetchClassOptions(), [fetchClassOptions]);

//   /* ---------------- MARK AS SUBMITTED ---------------- */
//   const markAsSubmitted = async (testId) => {
//     const { error } = await supabase.from("tests").update({ is_submitted: true }).eq("id", testId);
//     if (!error) fetchTests(page);
//   };
...
//             <button
//               onClick={() => {
//                 setSearchTerm(""); setSelectedClass(""); setSelectedDate(""); setSelectedStatus("");
//               }}
...
//                   {test.test_title}
//                 </h2>
//                 <p><strong>Class:</strong> {test.class?.class_name || "Unknown"}</p>
//                 <p><strong>Arms:</strong> {test.test_arms?.map(a => a.arm?.arm_name).join(", ") || "None"}</p>
//                 <p className="text-sm"><strong>Date:</strong> {formatDate(test.test_date)}</p>
