...
const RecordExams = () => {
  ...
  const examId = localStorage.getItem("record_exam_id");
  ...
  const assignedArmIds = exam?.arm_id === null 
    ? []
    : exam?.exam_arms?.map(ea => ea.arm_id) || [];

  const { 
    data: studentsData, 
    isLoading: studentsLoading,
  } = useQuery({
    queryKey: ['students', exam?.class_id, assignedArmIds, studentPage, debouncedSearch],
    queryFn: async () => {
      let query = supabase
        .from("students")
  ...
  // ✅ FIXED: Fetch ALL submissions for this exam (not just current page)
  const { 
  ...
  const saveMutation = useMutation({
    mutationFn: async ({ studentId, score }) => {
      ...
      const { data, error } = await supabase.rpc('record_exam_score', {
        p_exam_id: parseInt(examId),
    ...
    onSuccess: (data, variables) => {
      // Update the all-submissions cache directly
      queryClient.setQueryData(
        ['exam-submissions-all', examId],
        (old) => {
          const newSub = {
  ...
  const batchSaveMutation = useMutation({
    mutationFn: async () => {
      ...
      const promises = entries.map(([studentId, score]) => 
        supabase.rpc('record_exam_score', {
          p_exam_id: parseInt(examId),
          p_student_id: studentId,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['exam-submissions-all', examId]);
      setPendingScores({});
      setValidationErrors({});
    },
    onError: (error) => {
  ...
  // Build map from ALL submissions (not just current page)
  const submissionsMap = Object.fromEntries(
    (allSubmissionsData || []).map(s => [s.student_id, s])
  );
  ...
  // Progress counts submissions with valid scores across ALL pages
  const scoredCount = (allSubmissionsData || []).filter(
    ...
    return (
      <div className="w-full max-w-5xl mx-auto px-4 py-6">
        <div className="bg-red-100 text-red-800 border border-red-300 rounded-md p-4">
          <h2 className="font-bold text-lg mb-2">⚠️ No Exam Selected</h2>
          <p>Please select an exam from the history page.</p>
        </div>
      </div>
    );
  }
  ...
  if (examError) {
    return (
      <div className="w-full max-w-5xl mx-auto px-4 py-6">
        <div className="bg-red-100 text-red-800 border border-red-300 rounded-md p-4">
          <h2 className="font-bold text-lg mb-2">⚠️ Error Loading Exam</h2>
          <p>{examError.message}</p>
        </div>
      </div>
    );
  }
        ...
        <>
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                ...
                {exam?.duration_minutes && (
                  <span className="ml-4">Duration: {exam.duration_minutes} mins</span>
                )}
              </p>
              <p className="text-sm text-gray-500 mt-1">
                {assignedArmIds.length > 0 ? (
                  <>
                    Arms: {exam.exam_arms.map(ea => ea.arm?.arm_name).join(", ")}
          ...
          <div className="mb-4">
            <div className="flex justify-between text-sm text-gray-600 mb-1">
              <span>
                {submissionsLoading && !allSubmissionsData
                  ? "Loading progress..."
                  : `Progress: ${scoredCount} of ${totalStudents} total scored`
                }
              </span>
            ...
            <div className="bg-gray-200 rounded-full h-2">
              <div 
                ...
                style={{ 
                  width: totalStudents > 0 && !submissionsLoading
                    ? `${(scoredCount / totalStudents) * 100}%` 
                    : '0%' 
                }}
              ></div>
            </div>
          ...
          <ul className="space-y-4">
            {students.map((student) => {
              ...
              // FIX: null-safe check so score=0 is treated as recorded
              const isRecorded = (existingScore !== null && existingScore !== undefined) && !pendingScore;
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
                    {isRecorded ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => handleScoreChange(student.id, existingScore.toString())}
